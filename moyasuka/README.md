...
- **動画合成パイプラインをLINEチャットUIに全面刷新**(`line_chat.py`)。オーナーからの実映像フィードバックを2ラウンド反映済み(①メッセージのコンパクト化・連投グループ化 ②内容強化・1分尺化・ナレーションカード廃止・チャットパネル縮小して上部配置)。台本01を各バージョンで実レンダリングし、複数時点のフレームを目視確認済み。残るは実際のナレーション音声の差し込みのみ
- 台本: 3本すべてLINEチャット形式(ナレーションなし、約50〜58秒)に書き直し済み(`scripts/01-sample.md`〜`03-coworker.md`)、ネタ帳に残り7本(`docs/projects/moyasuka/content-backlog.md`)
