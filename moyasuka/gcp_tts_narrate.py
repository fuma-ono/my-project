...
from moyasuka.line_chat import HARSH_TRIGGER_WORDS, PAUSE_SECONDS, estimate_arrivals, is_pause_only, parse_chat_script, strip_emoji, timing_fingerprint, total_seconds_for
...
SCRIPT_VOICE_OVERRIDES: dict[str, dict[str, int]] = {
    ...
    # 台本08(2回目、2026-08-20): オーナーが一人称「俺」の台詞を直接
    # 提供、男性主人公として書かれているため「私」を男声(id11)に。
    # 「上司」はCHARACTER_SPEAKER_IDSの全体マッピングでは台本03の女性
    # 上司役(id2)を指すため、この台本限定でid12(男声)に上書き——id11
    # (私)と被らない。
    "08-mass-resignation": {"私": 11, "上司": 12},
}
...
def build_narration(script_path: str, out_wav: str, out_durations_json: str) -> float:
    ...
    with TemporaryDirectory() as tmp:
        for i, item in enumerate(items):
            ...
            # 絵文字は視覚的な身振りであって単語ではない(strip_emoji参照)
            # ——渡したままだとGoogle Cloud TTSが読み上げてしまう。
