...
# 自体は変えていない。
# Round 5 (2026-08-20、台本09): 「最初の通知音からチャットの開始まで間が
# あるからすぐチャットが始まるようにして」— Round 4の0.2秒の余裕が、
# チャイムを最後まで聞いた後の"間"としてまだ感じ取れていた。チャイム自体
# の実測尺(line_notification.mp3、約1.907秒)ぎりぎりまで余裕を削り、
# 0.2秒→0.05秒に短縮(NOTIFICATION_DELAY 0.2 + チャイム約1.907 + 0.05
# ≈ 2.15秒)。チャイムが鳴り終わった直後にほぼゼロラグで最初のセリフが
# 始まる計算——不変条件(チャイムに被らない)自体は据え置き。
# Round 6 (2026-08-20、同日): 「だから通知音〜チャット開始の間がながいって。
# かさなっていいから即チャットが開始するようにして」— Round 5はまだ
# 「チャイムに被らない」という不変条件そのものを死守していたため、体感の
# 間は消えていなかった。今回はその不変条件自体を明示的に破棄する指示。
# チャイムの再生完了を待たず、COLD_OPEN_SILENT_GAP(無音の先頭項目向け、
# 0.4秒)と同じ値に統一——最初の項目が写真でもセリフでも、同じ近い間隔で
# 始まる。ナレーションはチャイムの音とほぼ全区間重なることになるが、
# それが今回の明示的な指示。
NOTIFICATION_DELAY = 0.2
LEAD_IN_SECONDS = 0.4  # 最初に「実際に喋る」項目が守るべき下限。チャイムの再生完了は待たない(Round 6参照) — 無音の先頭項目にはCOLD_OPEN_SILENT_GAPが同じ値で対応
