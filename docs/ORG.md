...
- アプリエンジニア — `app/`(Focus & Sleep Sounds、新規投資は縮小・保守のみ)と `expense-app/`(「サクッと経費」、個人事業主・副業ワーカー向け経費/収支管理アプリ。2026-08-01よりこちらを優先開発、`docs/marketing/2026-08-app-pivot-decision.md`)の実装
...
- ライター(note担当) — `note-articles/` のnote記事の執筆。専任の「note編集部」(編集長・ベテラン作家2人・中堅編集者2人・ベテランマーケター・天才作家)を編成済み。体制・役割分担は `note-articles/TEAM.md`
...
数字が伸び悩んだ場合、まずここで外部要因の仮説(競合/アルゴリズム変化/市場トレンド)を立てる。**note・SNSはアジャイル(週次スプリント)で運用する**(2026-08-01〜、`docs/marketing/2026-08-note-sns-growth-strategy.md`)。計画→公開→計測(スキ数・閲覧数)→振り返りを毎週回し、伸びた記事の型を次週のネタ選定に反映する。ウォーターフォールは不採用(実測フィードバックを反映できないため)。SNS(X)でのnote記事告知文のドラフト作成、投稿キューの管理も担当。

**GROWTH-HQ(kashikari担当、2026-09-11新設)**: マーケティング部・グロース/運用部・データ分析部の合同ミッションとして、貸し借りアプリ「kashikari」(アプリ本体は開発部が別ブランチで開発中)の「認知→インストール→初回利用→招待→継続→収益」の成長ループを検証・改善する。単純な再生数・フォロワー数はKPIとせず、Install/Activation/Invitation Loop/Retention/LTVで判断する。方針の詳細・ボトルネック診断基準・撤退基準は `docs/projects/kashikari/growth-strategy.md` を参照。
