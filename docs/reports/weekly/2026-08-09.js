...
async function main() {
  ...
  // ============================================================
  {
    ...
    s.addText("第2回 · 2026年8月9日", {
      x: 0.9, y: 3.75, w: 8, h: 0.5,
      fontFace: FONT_BODY, fontSize: 18, color: TEXT_DIM,
    });
    s.addText("BGM動画 / アプリ / note記事 / モヤスカ(新) — 4事業の運営状況を社長よりご報告します", {
  ...
  // ============================================================
  {
    ...
    s.addText("2026-08-04 〜 08-09", { x: 0.6, y: 1.05, w: 6, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const tiles = [
      { icon: "sparkle", n: "第4事業", label: "「モヤスカ」始動\nチャンネル開設・台本3本", bg: NAVY_DEEP },
      { icon: "phone", n: "初成功", label: "iPad Shortcuts自動化を\n実機で動作確認・実音声完成", bg: NAVY_DEEP },
      { icon: "music", n: "+1本", label: "BGM Shortsを追加公開\n(累計Shorts 2本)", bg: NAVY_DEEP },
    ...
    s.addText("今週最大の成果: 新事業「モヤスカ」でナレーション自動化(iPad Shortcuts + VOICEVOX API)がオーナーの実機で動き、初めて実音声入りの動画が完成しました。",
  ...
  // ============================================================
  {
    ...
    s.addText("4事業体制に。モヤスカは投稿目前、他3事業はオーナー対応待ちが中心", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });

    const cards = [
      {
        icon: "music", title: "BGM動画", pill: "公開中", pillColor: GOOD,
        rows: ["長尺2本+Shorts2本(今週+1本)", "週次(長尺)+週3(Shorts)で自動運用", "本日のShorts自動公開は権限ブロックで失敗(p.5)"],
      },
      {
        icon: "phone", title: "アプリ", pill: "申請待ち", pillColor: WARN,
        rows: ["開発完了、ストア未申請", "Apple/Google/Expoアカウント待ち", "変化なし(先週から継続)"],
      },
      {
        icon: "article", title: "note記事", pill: "公開中", pillColor: GOOD,
        rows: ["公開本数: 7本(先週と同数)", "下書き08・09を追加、投稿待ち", "オーナーの投稿操作待ち"],
      },
      {
        icon: "sparkle", title: "モヤスカ(新)", pill: "投稿目前", pillColor: AMBER,
        rows: ["LINEドラマ×AI音声のShortsチャンネル", "台本3本・チャンネル開設済み", "実音声入り動画が初めて完成(p.4)"],
      },
    ];
  ...
  // ============================================================
  {
    const s = pres.addSlide();
    lightBg(s);
    s.addText("今週の主な取り組み: モヤスカ立ち上げ", { x: 0.6, y: 0.45, w: 11.5, h: 0.6, fontFace: FONT_HEAD, fontSize: 26, color: INK, bold: true });

    const items = [
      { icon: "sparkle", title: "チャンネル開設・動画の骨格を完成", body: "「モヤスカ」(@moyasuka)開設。背景映像・LINEチャットUI・効果音・ポップなBGMまで、フィードバックを10回以上反映して仕上げ。" },
      { icon: "phone", title: "ナレーション自動化を実機で確立", body: "PCなしのiPhone/iPadという制約の中、iPad Shortcuts + VOICEVOX APIで全セリフを自動生成する仕組みをオーナーと一緒に構築。ボット判定・レート制限・0埋め問題など、実機ならではの壁を一つずつ解決。" },
      { icon: "check", title: "初めて実音声入りの動画が完成", body: "台本01(30行)を実際に自動生成し、組み立てた動画が完成。オーナーへ送付し感想待ち。台本02・03の収録も進行中。" },
      { icon: "build", title: "最後の1手間もさらに削減中", body: "生成したファイルをこちらに送る「最後の1回」も自動化できないか検討し、Dropbox連携を接続。読み取り確認済み、仕上げ中。" },
    ];
  ...
  // ============================================================
  {
    ...
    const rows = [
      {
        icon: "warn", title: "Shorts自動公開が権限ブロックで失敗(新規)",
        steps: ["本日(週3回定例)のShorts自動公開が失敗", "OAuth期限切れではなく、コマンド実行自体が権限クラシファイアに拒否された", "回避策は試さず停止し、Push通知で報告済み", "publish_shortsの実行を許可するBash権限ルールの追加が必要"],
      },
      {
        icon: "phone", title: "既存の保留事項(継続)",
        steps: ["TikTok: GitHub Pages有効化待ち(ドメイン検証)", "Instagram: developers.facebook.comへのアクセス不可が継続", "アプリ: Apple/Google/Expoアカウント登録待ち", "変化なし、引き続きオーナー対応待ち"],
      },
    ];
  ...
  // ============================================================
  {
    ...
    s.addText("4つのRoutineが無人稼働中(1件は本日、権限ブロックで停止中)", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const routines = [
      { icon: "music", name: "BGM長尺動画", freq: "週次(火曜)", detail: "生成→レンダリング→アップロード→処理完了確認→公開" },
      { icon: "warn", name: "BGM Shorts", freq: "週3回(火木土)", detail: "本日の実行が権限ブロックで失敗、対応依頼中(p.5)" },
    ...
    routines.forEach((r) => {
      ...
      s.addText(r.detail, { x: 1.65, y: y + 0.53, w: 7.6, h: 0.45, fontFace: FONT_BODY, fontSize: 11, color: INK_DIM });
  ...
  // ============================================================
  {
    ...
    const notes = [
      ...
      "モヤスカ: 投稿すらこれから。ゼロベースの新チャンネルのため障壁は他事業と同水準",
    ];
    let ny = 1.7;
    notes.forEach((n) => {
  ...
  // ============================================================
  {
    ...
    const asks = [
      { n: "1", title: "モヤスカ: 送付済み動画の確認+台本02・03の収録継続", body: "実音声入り動画(台本01)の方向性を確認してください。問題なければ初回投稿の準備が整います。台本02・03のナレーションも同じ手順で収録をお願いします。" },
      { n: "2", title: "Shorts自動公開のBash権限ルール追加", body: "publish_shortsの実行を許可する権限ルールの追加をお願いします。解決しない限り、週3回の自動公開が同じ理由で失敗し続けます。" },
      { n: "3", title: "アプリ: 各種アカウント登録 / Instagram・TikTokの継続対応", body: "Apple Developer / Google Play Console / Expoアカウント登録。Instagram の2FA状況、TikTok用GitHub Pages有効化も引き続きお願いします。" },
    ];
    let y = 1.65;
    asks.forEach((a) => {
  ...
  // ============================================================
  {
    ...
    const plan = [
      { icon: "sparkle", text: "モヤスカ: 台本02・03のナレーション収録を完了させ、初回投稿を実施" },
      { icon: "music", text: "長尺動画1本・Shorts最大3本を自動公開(権限ブロックが解消し次第)" },
  ...
  await pres.writeFile({ fileName: `${__dirname}/2026-08-09.pptx` });
  console.log("done");
}
