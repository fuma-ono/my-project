...
async function main() {
  ...
  // ============================================================
  {
    ...
    s.addText("2026-09-07 〜 09-14", { x: 0.6, y: 1.05, w: 6, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const tiles = [
      { icon: "warn", n: "16日間停止", label: "BGM・モヤスカのYouTube公開が\n8/29から継続して停止中(状況変化なし)", bg: NAVY_DEEP },
      { icon: "check", n: "検知は正常継続", label: "毎日のOAuth事前チェックが\n無言失敗なく正しくスキップを継続", bg: NAVY_DEEP },
      { icon: "article", n: "在庫6本を維持", label: "note記事は2回の投稿完了を検知し\n#30・31を補充、安定稼働", bg: NAVY_DEEP },
    ...
    s.addText("今週の要点: 先週報告した認証情報消失(client_secret.json)は今週も未解決のまま、BGM・モヤスカの公開停止が16日目に入りました。新しい問題は発生しておらず、毎日のOAuth事前チェックは引き続き正しく検知・スキップを継続(無言失敗なし)。この間もnote記事は影響を受けずに稼働を続け、#24・25の投稿完了を自動検知した後、#30・31を新規生成して在庫6本を維持しています。オーナーへの依頼事項に変化はなく、client_secret.jsonの再発行のみをお待ちしている状態です。",
  ...
  // ============================================================
  {
    ...
    s.addText("BGM・モヤスカは停止継続(16日目)、note記事は今週も正常稼働", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });
    ...
    const cards = [
      ...
      },
      {
        ...
        rows: ["公開27本、下書き在庫6本を維持", "#30「壁の薄い部屋」等を新規補充", "オーナーの投稿操作待ち"],
      },
      {
  ...
  // ============================================================
  {
    const s = pres.addSlide();
    lightBg(s);
    s.addText("今週の主な取り組み: 平常運用の継続とnote記事の安定供給", { x: 0.6, y: 0.45, w: 11.5, h: 0.6, fontFace: FONT_HEAD, fontSize: 24, color: INK, bold: true });

    const items = [
      { icon: "article", title: "note記事を2回に分けて4本新規生成、在庫6本を維持", body: "#24・25の投稿完了をRSS自動検知した後、柱A(始める前の3分/壁の薄い部屋)を#29・30として生成。続けて#31「作業終わりのタイマー活用」も補充し、柱Aのバックログを順に消化している。" },
      { icon: "sparkle", title: "モヤスカ台本の週次チェックは新規作成なしで完了", body: "書き上がっている台本が目標5本を大きく上回る13本のストックがあるため、今週の生成Routineは新規作成をスキップ。公開自体は認証復旧待ちのため、ストックはそのまま積み上がっている。" },
      { icon: "check", title: "毎日のBGM Shorts検知は16日連続で正常動作", body: "OAuth残り日数の事前チェックが認証情報なしの状態を毎回正しく検知・スキップし続けている。無言で失敗する既知のパターン(2026-08-08)は一度も再発していない。" },
      { icon: "warn", title: "BGMコンテンツ戦略の週次判断は3回連続で見送り", body: "新規公開自体ができない状態が続いているため、piano_hisaishi_styleの実績判定を含む判断基準の適用を今回も保留。復旧後の最初の週次レビューでまとめて確認する方針を維持している。" },
    ];
  ...
  // ============================================================
  {
    ...
    s.addText("最優先: client_secret.jsonの再発行(オーナー対応が必須、継続16日目)", { x: 0.6, y: 1.05, w: 10.5, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });
  ...
  // ============================================================
  {
    ...
    const routines = [
      ...
      { icon: "warn", name: "BGM Shorts", freq: "毎日", detail: "毎日の事前OAuthチェックが正しく検知・スキップ、16日連続で無言失敗なし" },
      { icon: "article", name: "note下書き", freq: "週2回(月木)", detail: "影響なく正常稼働、#30・31を補充し在庫6本を維持" },
      { icon: "sparkle", name: "モヤスカ台本生成", freq: "週次", detail: "ストック13本で目標超過のため今週は新規作成をスキップ" },
      { icon: "schedule", name: "経営レビュー", freq: "週次(月曜)", detail: "BGMコンテンツ戦略の判断は3回連続で見送り、公開停止中のため" },
    ];
    let y = 1.7;
    routines.forEach((r) => {
  ...
  // ============================================================
  {
    ...
    const plan = [
      ...
      { icon: "article", text: "note下書きRoutineが柱Aの残バックログを消化しつつ在庫を維持" },
  ...
  // ============================================================
  {
    ...
    s.addText("継続16日目、届き次第その日のうちにBGM・モヤスカの公開を再開します。", { x: 0.9, y: 3.75, w: 10, h: 0.5, fontFace: FONT_BODY, fontSize: 14, color: TEXT_DIM });
  ...
  await pres.writeFile({ fileName: `${__dirname}/2026-09-14.pptx` });
  console.log("done");
}
