...
async function main() {
  ...
  // ============================================================
  {
    ...
    s.addText("2026-08-24 〜 08-31", { x: 0.6, y: 1.05, w: 6, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const tiles = [
      { icon: "sparkle", n: "8プリセット目", label: "ピアノ(久石譲風)を新規導入\nSTEP1〜6完走・公開まで完了", bg: NAVY_DEEP },
      { icon: "check", n: "自動運転", label: "コンテンツ戦略の判断も\n完全自動化(オーナー指示)", bg: NAVY_DEEP },
      { icon: "music", n: "+4本", label: "BGM長尺1・Shorts3を公開\n(累計 長尺8・Shorts11)", bg: NAVY_DEEP },
      { icon: "warn", n: "2件", label: "OAuth再認証が必要\n(BGM・モヤスカ、承認依頼中)", bg: NAVY_DEEP },
    ];
    ...
    s.addText("今週最大の出来事: オーナー指示「BGMにピアノを増やしたい」を起点に、久石譲風の新プリセットを音作りから公開まで一気に仕上げました。試作を何度も送って音色・明るさ・構成のフィードバックをいただき(電子音的→アコースティック、暗い→明るい、単調ループ→A/B構成)、公開直前の自前チェックで見つけた音声バグも修正。1時間フル尺は一度unlistedで公開してオーナーに実際に視聴確認いただいてからpublicに切り替える、という慎重な手順を踏みました。その後オーナーから「完全自動化で」と指示をいただき、これ以降の新バリエーション追加・3時間版検討は判断も含めて自動で進める体制に切り替えています。",
  ...
  // ============================================================
  {
    ...
    s.addText("BGMが8プリセット目(ピアノ)を公開、2事業でOAuth再認証待ち", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });

    const cards = [
      {
        icon: "music", title: "BGM動画", pill: "公開中", pillColor: GOOD,
        rows: ["長尺8本+Shorts11本(今週+4本)", "8プリセット目(ピアノ)公開(p.4)", "OAuth残り1.2日・再認証依頼中"],
      },
      {
      ...
      },
      {
        ...
        rows: ["公開本数: 20本、下書き在庫6本", "週2回の自動下書きが安定稼働", "オーナーの投稿操作待ち"],
      },
      {
        icon: "sparkle", title: "モヤスカ", pill: "要対応", pillColor: WARN,
        rows: ["台本13本ストック(公開5本)", "OAuth期限切れ・再認証依頼中", "台本10以降の方向性確認待ち"],
      },
    ];
  ...
  // ============================================================
  {
    const s = pres.addSlide();
    lightBg(s);
    s.addText("今週の主な取り組み: ピアノプリセット導入と完全自動化", { x: 0.6, y: 0.45, w: 11.5, h: 0.6, fontFace: FONT_HEAD, fontSize: 24, color: INK, bold: true });

    const items = [
      { icon: "music", title: "ピアノプリセットをゼロから合成、久石譲風に仕上げ", body: "サンプル音源不使用、倍音合成でアコースティックピアノを再現(ハンマー打鍵ノイズ・弦のうなり・打弦位置の倍音特性まで実装)。試作を重ね「電子音的→アコースティック」「暗い→明るい」「単調ループ→A/B2区間構成」とオーナー要望に合わせて改善。" },
      { icon: "check", title: "動画にカウントダウンタイマーを新規実装", body: "1時間から1秒ずつ減っていく表示をffmpegのdrawtext数式展開だけで実現(フレーム生成不要)。実写背景+タイマー付きの1時間動画を、unlisted公開→オーナー視聴確認→public切り替えという慎重な手順で本番公開。" },
      { icon: "warn", title: "自前チェックで音声バグを発見・修正", body: "公開前の波形解析で、打鍵音のエンベロープにランプが無く毎音でクリックノイズが混入するバグを発見。修正して確認してから完成版を送付。ルーティンのOAuth期限切れ(BGM残り1.2日、モヤスカは期限切れ)も検知し再認証依頼中。" },
      { icon: "sparkle", title: "コンテンツ戦略の判断も完全自動化へ移行", body: "オーナー指示「あとは自動で判断して...完全自動化で」を受け、新バリエーション追加・3時間版検討の判断基準を明文化し週次レビューに組み込み。以後は事後報告のみに変更(判断基準はp.6参照)。" },
    ];
  ...
  // ============================================================
  {
    ...
    s.addText("OAuth再認証が2事業で同時に必要(新規)", { x: 0.6, y: 1.05, w: 10, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });

    const rows = [
      {
        icon: "warn", title: "YouTube OAuth再認証(新規・2件)",
        steps: ["BGM: 残り1.2日まで低下、デバイスコード発行済み", "モヤスカ: 既に期限切れ、デバイスコード発行済み", "どちらもPush通知でURL・コードを承認依頼中", "テストモードのため約7日ごとに繰り返し発生"],
      },
      {
        icon: "phone", title: "既存の保留事項(継続)",
        steps: ["アプリ: Apple/Google/Expoアカウント登録待ち", "モヤスカ: 台本10以降の方向性、オーナー確認待ち", "BGM: 高評価・コメントは出始めたが件数少(p.7)", "TikTok/Instagramは審査・接続待ちで保留"],
      },
    ];
  ...
  // ============================================================
  {
    ...
    s.addText("公開頻度は元々無人。今週からコンテンツ戦略の判断も自動化", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const routines = [
      { icon: "music", name: "BGM長尺動画", freq: "週次(火曜)", detail: "生成→レンダリング→アップロード→処理完了確認→公開" },
      { icon: "check", name: "BGM Shorts", freq: "毎日", detail: "週3回→毎日に変更済み。OAuth事前チェック付きで安定稼働" },
      { icon: "article", name: "note下書き", freq: "週2回(月木)", detail: "在庫を自動維持、テーマバックログから自動選定" },
      { icon: "sparkle", name: "モヤスカ台本生成", freq: "週次", detail: "5本ストック維持、台本ごとに個別スケジュールで公開" },
      { icon: "schedule", name: "経営レビュー(拡張)", freq: "週次(月曜)", detail: "新規: BGM新バリエーション・3時間版追加の判断も自動実行" },
    ];
    let y = 1.7;
    routines.forEach((r) => {
  ...
  // ============================================================
  {
    ...
    s.addText("現時点で¥0。エンゲージメント導線の効果が初めて確認できた", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });
    ...
    const notes = [
      "YouTube: Shorts(62〜313回)が長尺(2〜123回)を引き続き大きく上回る。8/25以降追加したコメント誘発質問文の効果が初確認(1本で高評価3・コメント2、導入前は全動画0件)",
      "アプリ: Apple/Google/Expoアカウント登録待ちで未申請",
      "note: 公開20本・下書き在庫6本。有料マガジンはフォロワー・反応が育ってから移行予定",
      "モヤスカ: 台本13本ストックも公開は5本のまま。台本10以降の方向性がオーナー確認待ち",
    ];
    let ny = 1.7;
    notes.forEach((n) => {
  ...
  // ============================================================
  {
    ...
    const asks = [
      { n: "1", title: "BGM・モヤスカ: YouTube OAuth再認証の承認(2件)", body: "両事業とも残り日数が閾値を下回った(モヤスカは期限切れ)ため、デバイスコードを発行済みです。Push通知記載のURL・コードで承認をお願いします。" },
      { n: "2", title: "アプリ: 各種アカウント登録", body: "Apple Developer / Google Play Console / Expoアカウント登録。完了次第、実際のストア申請・EAS Buildの準備に進みます。" },
      { n: "3", title: "モヤスカ: 台本10以降の方向性", body: "既に13本ストックがありますが、公開自体は5本で止まっています。1日1本ペースで公開を再開してよいか、方向性のご指示をお願いします。" },
    ];
    let y = 1.65;
    asks.forEach((a) => {
  ...
  // ============================================================
  {
    ...
    const plan = [
      { icon: "music", text: "piano_hisaishi_styleの実績を継続観測。2週間ライブ経過時点(9月上旬)で新バリエーション追加を自動判断" },
      { icon: "check", text: "長尺動画1本・Shorts7本を自動公開。OAuth承認後は無人運用に復帰" },
      { icon: "article", text: "note下書きRoutineが柱A/柱Bの新作を自動作成、在庫を維持" },
      { icon: "sparkle", text: "モヤスカ: オーナー確認が得られ次第、台本10以降の公開を再開" },
  ...
  await pres.writeFile({ fileName: `${__dirname}/2026-08-31.pptx` });
  console.log("done");
}
