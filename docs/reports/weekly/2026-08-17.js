...
async function main() {
  ...
  // ============================================================
  {
    ...
    s.addText("2026-08-09 〜 08-17", { x: 0.6, y: 1.05, w: 6, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const tiles = [
      { icon: "sparkle", n: "モヤスカ", label: "背景映像・UI・音声を\n全面刷新、公開目前", bg: NAVY_DEEP },
      { icon: "music", n: "+1本", label: "BGM Shorts第4弾を公開\n(累計 長尺6・Shorts4)", bg: NAVY_DEEP },
      { icon: "check", n: "課題#28", label: "Shorts自動公開の\n権限ブロックを解消確認", bg: NAVY_DEEP },
    ...
    s.addText("今週最大の成果: モヤスカの動画パイプラインをオーナーの実際の視聴フィードバックをもとに何度も作り直し、背景映像(オーナー提供のAI生成写真によるKen Burns方式)・チャットUI・既読表示・通知音・ナレーションの抑揚まで、初回公開に耐える品質まで仕上げました。",
  ...
  // ============================================================
  {
    ...
    s.addText("モヤスカは公開目前、他3事業はオーナー対応待ちが中心", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: TEXT_DIM });

    const cards = [
      {
        icon: "music", title: "BGM動画", pill: "公開中", pillColor: GOOD,
        rows: ["長尺6本+Shorts4本(今週+1本)", "3本のタイトル・サムネイルを改訂", "OAuth再認証をオーナーに依頼中(p.5)"],
      },
      {
      ...
      },
      {
        ...
        rows: ["公開本数: 10本、下書き在庫6本", "週2回の自動下書きが安定稼働", "オーナーの投稿操作待ち"],
      },
      {
        icon: "sparkle", title: "モヤスカ", pill: "公開目前", pillColor: AMBER,
        rows: ["台本5本完成、背景映像を実写化", "既読・通知音・抑揚まで仕上げ済み", "台本01の最終確認待ち(p.4)"],
      },
    ];
  ...
  // ============================================================
  {
    const s = pres.addSlide();
    lightBg(s);
    s.addText("今週の主な取り組み: モヤスカの品質仕上げ", { x: 0.6, y: 0.45, w: 11.5, h: 0.6, fontFace: FONT_HEAD, fontSize: 26, color: INK, bold: true });

    const items = [
      { icon: "sparkle", title: "背景映像を4回作り直し、実写に着地", body: "ボール物理演算→パララックス→マイクラ風ブロック→手続き型の波シェーダーと試作を重ね、最終的にオーナー提供のAI生成写真によるKen Burns(ゆっくりパン+ズーム)方式に確定。" },
      { icon: "phone", title: "ナレーションをGCP Cloud TTSに全面移行", body: "iPad手動収録から完全自動化。台本編集後にナレーションの再生成を忘れて音声とチャットがズレる事故が発生したため原因を特定・修正し、SSMLでセリフに抑揚をつける処理も追加。" },
      { icon: "check", title: "チャットUI・通知音・既読表示を仕上げ", body: "白いパネル・ヘッダーを撤去し吹き出しが背景に直接浮かぶ構成に刷新。冒頭に実際のLINE通知音、送信メッセージに既読表示を追加。すべてオーナーの実映像レビューを反映。" },
      { icon: "build", title: "BGM側もタイトル・サムネイルを改訂", body: "3本の長尺動画のタイトル・サムネイルをオーナー指示で改訂(明朝体・中央揃えのスタイルに統一)。Shorts自動公開の権限ブロック問題(課題#28)も解消を確認。" },
    ];
    ...
    const rows = [
      {
        ...
        steps: ["残り日数が2.6日まで低下し、事前チェックで検知", "デバイスコードを発行し承認をPush通知で依頼済み", "承認いただき次第、自動で反映されます", "テストモードのため約7日ごとに繰り返し発生"],
      },
      {
        icon: "phone", title: "既存の保留事項(継続)",
        steps: ["アプリ: Apple/Google/Expoアカウント登録待ち", "モヤスカ: 台本01最終版のオーナー確認待ち", "BGM: 全動画で高評価・コメントが依然ゼロ(p.7)", "TikTok/Instagramは審査・接続待ちで保留"],
      },
    ];
  ...
  // ============================================================
  {
    ...
    s.addText("5つのRoutineが無人稼働中、Shorts権限ブロックは解消確認済み", { x: 0.6, y: 1.05, w: 11, h: 0.35, fontFace: FONT_BODY, fontSize: 13, color: INK_DIM });

    const routines = [
      { icon: "music", name: "BGM長尺動画", freq: "週次(火曜)", detail: "生成→レンダリング→アップロード→処理完了確認→公開" },
      { icon: "check", name: "BGM Shorts", freq: "週3回(火木土)", detail: "課題#28(権限ブロック)を解消確認、直近も正常稼働" },
      { icon: "article", name: "note下書き", freq: "週2回(月木)", detail: "在庫6本を維持、テーマバックログから自動選定" },
      { icon: "sparkle", name: "モヤスカ台本", freq: "週次(水曜)", detail: "在庫5本を自動維持、新規テーマも自動追加" },
  ...
  // ============================================================
  {
    ...
    const notes = [
      ...
      "モヤスカ: 初回投稿すらこれから。ゼロベースの新チャンネルのため障壁は他事業と同水準",
    ];
    let ny = 1.7;
    notes.forEach((n) => {
  ...
  // ============================================================
  {
    ...
    const asks = [
      { n: "1", title: "モヤスカ: 台本01最終版の確認", body: "実写Ken Burns背景・既読・通知音・ナレーション抑揚まで反映した最終版をお送りしています。問題なければ、既存のunlisted動画を差し替えて初回公開に進みます。" },
      { n: "2", title: "BGM: YouTube OAuth再認証の承認", body: "残り日数が閾値を下回ったため、デバイスコードを発行済みです。Push通知記載のURL・コードで承認をお願いします。" },
  ...
  // ============================================================
  {
    ...
    const plan = [
      { icon: "sparkle", text: "モヤスカ: オーナー確認が取れ次第、台本01を初回公開。台本02以降の制作も継続" },
      { icon: "music", text: "長尺動画1本・Shorts最大3本を自動公開。BGMの高評価/コメント導線強化を検討" },
      { icon: "article", text: "note下書きRoutineが柱A/柱Bの新作を自動作成、在庫6本を維持" },
  ...
  await pres.writeFile({ fileName: `${__dirname}/2026-08-17.pptx` });
  console.log("done");
}
