...
**作成方針(2026-09時点)**: 詳細設計の仕様・設計判断はHQ(楓真＋ChatGPT)側で行う。本書はClaude Code側で確定した設計ではなく、HQが詳細設計を作成する際の**インプット情報**として、要件定義書v1.2・概要設計書v1.1・設計レビューで判明した内容を整理したもの。ここに書かれた内容はいずれも**未確定**であり、Claude Codeが独自に決定・実装したものではない。
...
- 要件定義書v1.2・概要設計書v1.1の内容(Snapshot/Revision分離・API配信権の必須化・時刻同期ルール・統計除外ルール・AI縮小・Scheduler/Queue/Worker構成)は、いずれも今回のHQ方針(4章のAdapter抽象化含む)と矛盾しておらず、そのまま詳細設計のベースにできると考えている。
