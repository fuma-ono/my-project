# FX Event Analyzer: 機能一覧 v1.1

**出典**: HQより2026-09-16共有。v1.1は、整合性確認3点(FEAT-045/224/028)へのHQ回答を反映したもの。実装は未着手。

**位置づけ**: 要件定義書v1.3・概要設計書v1.2・画面設計(ui-screens.md v1.0)の内容を、機能ID(FEAT-xxx)単位に分解したもの。今後の設計・実装で本書のIDを参照する。次フェーズでHQが「画面×機能」「画面×機能×API×DB」の対応表を作成する際の基礎資料となる。

## 変更履歴

- **v1.0**: 初版
- **v1.1**(今回): 整合性確認3点へのHQ回答を反映
...
「課金を後から追加する設計」ではなく、「最初からFree/Proへ拡張可能な設計にしておき、決済機構の実装タイミングだけ後で判断する」という方針(HQ回答、2026-09)。詳細は要件定義書v1.3 38〜55章・概要設計書v1.2 25章参照。
...
- **反映先**: 要件定義書v1.3 5.1節・概要設計書v1.2 5.4節、本書FEAT-045の記述を更新。
...
- **反映先**: 要件定義書v1.3 38〜55章・概要設計書v1.2 25章、本書9.1節(新設)・FEAT-227(新設)。
...
- **反映先**: 要件定義書v1.3 27章・概要設計書v1.2 5.1/5.2節。

### 15.2 整合性が確認できた箇所(参考)

- Surprise分析(FEAT-050〜054)は要件定義書v1.3 11章・概要設計書v1.2 9章の「Forecast欠損時はnull、0禁止」の方針と完全に一致
- AI分析(FEAT-200〜204)が全てP2であることは、要件定義書v1.3 12章・概要設計書v1.2 11章の「MVPでは出典付き事実要約のみ、自由生成AI解説は将来拡張」という縮小方針と一致
- 統計除外(FEAT-091)・分析可能件数表示(FEAT-090)は概要設計書v1.2 10.2/10.3節の母数表示ルールと完全に一致
- Snapshot/Revision管理(FEAT-046・143・144)は概要設計書v1.2 5.3/5.4節の設計と一致
