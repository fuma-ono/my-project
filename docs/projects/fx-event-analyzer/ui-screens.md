...
**位置づけ**: 本書はUI・画面遷移レベルの設計であり、要件定義書v1.2・概要設計書v1.1(データモデル・API・アーキテクチャ)を置き換えるものではない。画面とAPI/DBとの対応関係は、HQによる詳細設計の次のフェーズで確定する。
...
- **SCR-007→SCR-003の必須遷移**: 概要設計書v1.0 13.6節・要件定義書17章が要求する「Historical Event Detail→Indicator Detail」の必須遷移と一致
- **Home画面のデータ状態表示(取得中/未取得)**: 概要設計書v1.1 8.5節「data_statusをそのまま返し、クライアントはN/A表示する契約」と一致する設計思想
- **Historical Comparisonの母数表示**: 概要設計書v1.1 10.2節「分析可能件数/全件数を提示する」設計と完全に一致
- **ForecastなしイベントのHome表示**(モックアップのFOMC「予想 -」): 要件定義書v1.2 11.2節「Forecastなしはsurprise=null」の扱いと整合
