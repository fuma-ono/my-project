...
グラデーションの丸いアイコンバッジ(+きらめきの装飾)をヘッダーに追加し、「一緒にお金の管理を始めよう!」というサブタイトルを添えた。入力欄はプレースホルダーと例(「例: けんた」)を分け、送信ボタンとキャンセルボタンは横並びではなく縦に積む構成(送信=グラデーション、キャンセル=アウトライン)に変更した

**今回あえて実装しなかったもの**

- **通知ベル**: 参考画像にはヘッダーに🔔アイコンがあったが、通知を貯める仕組み(バックエンド)がまだ存在しないため、押しても何も起きない飾りのアイコンになってしまう。中身が伴ってから追加する
