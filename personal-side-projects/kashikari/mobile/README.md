...
- 「受け取る金額と支払う金額の間にある|を2つの間に配置して」: 2つの列(`grossCol`)が`flex:1`でカード幅いっぱいに引き伸ばされていたため、テキストの実際の幅と区切り線の位置がずれていた。列を中身の幅にフィットさせる形に変更し、区切り線が常にテキストとテキストの真ん中に来るようにした(右端に固定したいシェブロンは、その手前に`flex:1`のスペーサーを挟んで押し出す形にした)
