...
「リリース後の問い合わせ・障害・レビューを検知し、まとめてリリースしていく仕組みを作ってほしい。ただし介入は最小限にしたい(確認はするが自動化してほしい)」という依頼への対応。方針をユーザーと確認し、①週次で検知・トリアージ、②2週間ごとにまとめてリリース、③ストア提出だけは必ず最終確認を挟む、で合意した。

**全体の流れ**:
1. **検知層(週次)**: アプリ内フィードバック(新設)・Sentryのクラッシュ/エラーを見に行き、小さく明確なバグはその場で修正、大きめの要望はGitHub Issue化、個別対応が要るものは下書きを作って提示。週次で「今週の検知・対応サマリー」をユーザーに報告する
2. **リリース層(2週間ごと)**: たまった修正をまとめてビルドし、変更点のサマリーを提示した上で、ユーザーの最終GOを得てから`eas submit`する(ストア提出という後戻りしにくい行為だけは必ず確認を挟む)

**今回実装したのは検知経路の1つ、アプリ内フィードバックフォーム**:
- **`feedback`テーブル**(`schema.sql`): ユーザーが自由記述で「ご意見・不具合」を投稿できる投書箱。個人の自由記述を含みうるため、`analytics_events`より一段厳しいRLSにしてある — **投稿(insert)は本人のみ、閲覧(select)は誰にも許可しない**(投稿した本人ですら読み返せない)。運営側は`service role key`(RLSを無視できる管理者用の鍵)経由でのみ読む
- **`ops_state`テーブル**: 週次トリアージ・リリースバッチ処理が「前回どこまで見たか」を覚えておくための1行キーバリューストア。ローカルファイルに状態を持たせると開発環境のコンテナが再作成された時に消えてしまうため、Supabase側に永続化する設計にした
- **`src/lib/feedback.ts`**: `submitFeedback(userId, message)`。アプリバージョン・プラットフォームも一緒に記録する
- **`FeedbackModal.tsx`**: 設定画面から開く、シンプルな複数行テキスト入力のモーダル。送信後は「投稿した本人にも見えない」設計を踏まえ、送信できたことだけを分かりやすく伝える画面に切り替える
- **`SettingsScreen.tsx`**: 「💬 ご意見・不具合を報告」の行を追加。`onSubmitFeedback`は他のオプショナルpropsと同じ方針で、渡されない場合(デモモード)は行自体を出さない

**オーナー側の設定が必要**: 週次トリアージで私(Claude)がこのフィードバックを実際に読みに行くには、**Supabaseのservice role key**が要る。これは`.env`(アプリ本体に含まれる設定)には絶対に入れず、**この開発環境(Claude Code)側の環境変数**として設定する想定(アプリのビルド・実行には一切関与しない、この開発環境だけで使う鍵)。設定してもらい次第、週次・隔週の自動処理(Routine)を組む。

`npx tsc --noEmit`はクリーン。`EXPO_PUBLIC_DEMO_MODE=1`でのWeb版ビルド・Playwrightで、設定画面から「ご意見・不具合を報告」を開き、実際にメッセージを入力して送信→完了画面に切り替わることを確認した(デモ側の配線は検証用に一時的に追加し、確認後`git diff`で差分なしを確認して元に戻した)。デモモードでは行自体が表示されないことも確認済み。

## 実機バグ2件の修正(99回目)

実機確認で見つかった不具合2件への対応。

**① 設定画面でメールアドレス入力欄がキーボードに隠れる**

設定画面(`SettingsScreen.tsx`)は表示名・メールアドレス(`AuthMethods.tsx`)など入力欄が縦に並ぶ画面だが、`KeyboardAvoidingView`で囲われていなかった。`OnboardingScreen.tsx`・`LoginSheet.tsx`で既に使われている、このアプリで確立済みのパターン(`behavior={Platform.OS === 'ios' ? 'padding' : undefined}`)を同じように適用した。

**② 精算完了ポップアップの「閉じる」を押すと真っ白い画面のまま固まる**

原因は、「紹介する」ポップアップ(Alert)を閉じた直後、まだ閉じるアニメーション中のうちに、条件が揃っていればレビュー依頼ポップアップ(2つ目のAlert)を続けて出そうとしていたこと。iOSはネイティブのアラートを閉じている最中に次のアラートを提示しようとすると競合する(このファイル内の`ShareChannelSheet`/`InviteModal`で以前踏んだのと同じ種類の不具合、コード内のコメント参照)。`shouldShowReviewPrompt()`自体は一瞬で終わる非同期処理のため、実質「閉じるを押した直後」に近いタイミングで2つ目のAlertが呼ばれてしまっていた。1つ目の閉じるアニメーションが確実に終わるであろう400ms(このファイル内の別箇所で既に使われている値と同じ)待ってから出すように変更した。

いずれもAlert/ネイティブキーボードが絡むため、Web版のPlaywrightでは直接は再現・検証できない(Alertはreact-native-web上でネイティブダイアログとして描画されない)。`npx tsc --noEmit`はクリーン、設定画面の表示自体がレイアウト崩れなく動くことをWeb版で確認済み。次のビルドで実機での改善を確認してほしい。

## 実機バグ2件の再修正(99回目・実機再検証)

上記の修正版をビルドして実機確認したところ、**②(精算完了ポップアップのフリーズ)は再発、①(アイコンの遅さ)も改善していない**との報告を受け、両方をより根本的に直した。

**② 精算完了ポップアップ: Alertの連続呼び出しをやめ、独自Modalに置き換え**

400msのsetTimeoutで遅延させる対処では実機で解消しなかった(「閉じるを押すと白い画面のまま固まる」「紹介するを押しても何も出てこない」の両方が再現)。iOSのネイティブAlertを連続で呼ぶこと自体が実機では不安定と判断し、根本的に作り替えた:

- **`CelebrationModal.tsx`**(新規): 「精算完了」の紹介ダイアログ・レビュー依頼ダイアログ用の汎用モーダル。このアプリで既に確実に動いている「独自の`<Modal>`+`onDismiss`」方式(`InviteModal`/`ShareChannelSheet`と同じ、`<Modal>`が実際に閉じ終わった瞬間にだけ次の処理を行う)を採用し、Alertの連続呼び出し自体をやめた
- **`GroupScreen.tsx`**: `referralModalOpen`・`reviewModalOpen`の2つのstateと、`afterReferralDismissRef`(閉じるアニメーション完了後にやることを一時的に持ち回るref)で状態機械的に組み替えた。「閉じる」を押した場合はrefにレビュー依頼判定を、「紹介する」を押した場合は招待モーダルを開く処理を仕込んでおき、`onDismiss`(閉じ終わった瞬間)で実行する
- **`DemoGroupScreen.tsx`**: 同じ方式にミラー(レビュー依頼は本番のみの機能なので対象外、紹介→招待モーダルの部分だけ)
- **検証**: `mockData.ts`を一時的に書き換えて「あと1件で精算完了」の状態を作り、Web版のPlaywrightで実際に「受け取った」→精算完了ポップアップ表示→「紹介する」を押す→招待モーダルが正しく開く、という一連の流れをエラーなく確認した(Alertと違い独自Modalはreact-native-web上でも描画されるため、この部分は実際に動作検証できた)。検証用の書き換えは元に戻し、`git diff`で差分なしを確認済み

**① アイコンの表示が遅い: 本当のボトルネックは署名付きURLではなく写真そのものの大きさだった**

前回(署名付きURLのキャッシュ化)は往復のオーバーヘッドを減らしたが、**アイコン写真のアップロード時にリサイズ処理が一切なく、カメラ・アルバムから選んだフルサイズの写真(数MBになりうる)がそのままアップロードされていた**ことが分かった。24〜44pxの円形でしか表示しないアイコンに対して、毎回この大きなファイルをダウンロードし直すこと自体が本当のボトルネックだった。

- **`iconPhoto.ts`**: `expo-image-manipulator`を導入し、アップロード前に512×512へリサイズ・JPEG品質0.7で圧縮するようにした(選択時に`aspect:[1,1]`で正方形にクロップ済みなので歪まない)
...
**追記(対応完了、再提出済み)**: オーナーが②③を対応。②はSupabaseの「Confirm signup」「Magic Link」両テンプレートを`{{ .Token }}`表示に変更、保存済み。③はビルド`1.0.0 (202609080022)`をEASで作成・提出後、App Store Connectの「サブスクリプション」→ `kashikari_premium`グループの個別商品(Premium月額)ページから「審査用に追加」→ 表示されたバージョン選択でこのビルドを紐付け→ さらにグループ自体(`kashikari_premium`)も同じ下書きに追加、という手順で、**アプリバージョン・Premium月額・kashikari_premiumグループの3項目を一括で審査へ提出**した(提出日時2026-09-17 15:15、提出ID `8d990269-1d87-453d-80f1-4f0464338f5a`)。

**つまずいた点(次回同じ作業をする場合のメモ)**: サブスクリプショングループのトップページ(`kashikari_premium`一覧画面)にある「審査用に追加」ボタンは、何度押してもアプリバージョンを選択できるUIが出ず、「アプリバージョンを追加してください」というエラーのまま進めなかった。**個別のサブスクリプション商品(`Premium月額`)自身の詳細ページにある「審査用に追加」ボタンから操作したところ、初めてアプリバージョン(iOSアプリ1.0)が下書きに追加できた。** その後、グループのページに戻って改めて「審査用に追加」を押すと、同じ下書きに`kashikari_premium`グループも追加され、3項目揃って「審査へ提出」ボタンが押せるようになった。グループ経由ではなく個別商品経由でまず始めるのがコツ。

審査結果(承認/却下)が来たら、このセクションに追記する。

**追記(2026-09-18、再度却下、Guideline 3.1.2(c)。今度はコード修正あり)**: 上記対応後の提出は、まず3.1.2(EULAリンクがメタデータに無い)で却下→App Store ConnectのApp情報にカスタム使用許諾契約(全175地域)を登録し、説明文にも利用規約・プライバシーポリシーのリンクを追記して再提出したところ、**さらに3.1.2(c)で再度却下**された。今度の指摘は「メタデータだけでなく、**アプリ本体(購入画面自体)**にサブスクリプションの名称・期間・価格、および利用規約・プライバシーポリシーへの機能するリンクを表示すること」。`PremiumScreen.tsx`は価格(`priceString`)は表示していたが期間表記が無く、リンクも一切無かったため、これが原因と判明した。

- **`src/lib/legalLinks.ts`(新規)**: 利用規約・プライバシーポリシーのArtifact公開URLを定数化(`docs/legal/kashikari-terms-of-service.md`・`kashikari-privacy-policy.md`冒頭の「公開URL」と同じもの)
- **`PremiumScreen.tsx`**: 画面下部に「利用規約 ・ プライバシーポリシー」のリンクを追加(`Linking.openURL`で開く、既存の`ShareChannelSheet.tsx`と同じパターン)。isPremiumの状態に関わらず常に表示する
- **価格表示**: `priceString`(ストアの実価格)は期間を含まないため、`t.premium.priceWithPeriod(priceString)`で「月額」を付けて表示するように変更(フォールバック文言`t.premium.price`は元々「月額300円」と期間込みだったので変更不要)

`npx tsc --noEmit`はクリーン。`EXPO_PUBLIC_DEMO_MODE=1`のWeb版をPlaywrightで実際にPremium画面まで遷移し、「月額300円」の表示と「利用規約・プライバシーポリシー」リンクが見た目通り表示されることを確認した(リンク自体のタップ→外部ブラウザ遷移は、Web版のLinking.openURLの挙動がネイティブと異なるため未検証。実機/TestFlightでの最終確認が必要)。

**この修正はコード変更を伴うため、次の再提出には新しいビルド(`eas build` → `eas submit`)が必要。** ビルド後、前回と同じ手順(個別サブスクリプション商品ページから「審査用に追加」→ バージョン紐付け → グループも追加 → 審査へ提出)で再提出する。

**追記(審査完了、リリース済み)**: 上記対応後の再提出で審査を通過し、App Storeでリリースされた。

## リリース後の不具合報告2件(104回目)

リリース後、オーナーから2件の不具合が報告された。1件はコード側の実バグ(緊急度高)、もう1件はAdMob側の運用待ち(コード側の問題ではない)。

**① 緊急: 「ご意見・不具合を報告」フォームがキーボードに埋もれて操作不能**

`FeedbackModal.tsx`(設定画面から開く投書箱フォーム)は、`Modal`の直下に固定中央配置の`View`を置いているだけで、キーボード表示時に画面をずらす仕組みが一切無かった。文章を入力しようとして仮想キーボードが立ち上がると、画面下半分がキーボードに覆われ、**送信ボタン・キャンセルボタンの両方が画面外に押し出され、かつ背景をタップしても閉じる導線が無い**ため、ユーザーが完全に操作不能になっていた(実際に「詰んでしまう」と報告された)。同種の問題は`OnboardingScreen.tsx`・`LoginSheet.tsx`で既に`KeyboardAvoidingView`で対応済みだったが、`FeedbackModal.tsx`だけ対応が漏れていた。

- `FeedbackModal.tsx`: `View`だった外枠を`KeyboardAvoidingView`(`behavior="padding"`、iOSのみ)+`ScrollView`(`keyboardShouldPersistTaps="handled"`)に置き換え、`LoginSheet.tsx`と同じ構成にした。キーボード表示時は使える高さが縮む分だけカードが上にずれ、送信ボタンまで常に画面内(または軽くスクロールすれば届く位置)に収まるようにした

`npx tsc --noEmit`はクリーン。デモモード(`EXPO_PUBLIC_DEMO_MODE=1`)では`onSubmitFeedback`を渡していないためこの行自体が出ず、Web版のPlaywrightでは(仮想キーボードによるレイアウト変化も再現できないため)実際のキーボード回避動作までは確認できなかった。実装は既に本番で動作実績のある`LoginSheet.tsx`と同一パターンのため妥当と判断したが、**実機/TestFlightでの最終確認が必要**。

**② Google AdMobのアカウント承認は降りたが、広告が表示されない**

コード側(`AdBanner.tsx`・`ads.ts`・`GroupsScreen.tsx`での配置)を再確認したが、実装に問題は見つからなかった。広告ユニットID・アプリID(`app.json`の`androidAppId`/`iosAppId`)も前回(101回目)本番の値に差し替え済みのまま。**これはコード側の不具合ではなく、Google側の運用待ちの可能性が高い**:

- AdMobは「アカウント承認」と「個々の広告在庫が実際に配信され始めること」が別のタイミングで進む。アカウント承認直後でも、新規アプリ・新規広告ユニットは実際に広告が出始めるまで**数時間〜数日のタイムラグ**が生じることがある(Google公式にも明記されている挙動)。特にリリース直後でトラフィックが少ないアプリは「在庫が無い(no-fill)」判定になりやすい
- `AdBanner.tsx`には101回目で追加した診断ログ(`onAdFailedToLoad`・`onAdLoaded`)がそのまま残っているので、**実機をMacに繋いでMetro/Xcodeのログを見れば、`[AdBanner] failed to load:`の具体的なエラー内容(no-fillなのか、広告ユニットIDの設定ミスなのか)が分かる**。もしまだ`Account not approved yet`と出ているなら承認自体が反映しきっていない、`no-fill`ならただ在庫待ち、それ以外のエラーコードが出ていれば設定ミスの可能性がある、と切り分けられる
- 合わせて、AdMobダッシュボード側で「アカウント」だけでなく**広告ユニット自体(バナー・インタースティシャルそれぞれ)が「有効」になっているか**も確認してほしい(アカウント承認と広告ユニットの有効化は別項目)

コード側で今回変更した箇所は無い。**アプリの再ビルドは不要**(広告配信はサーバー側の設定で決まるため、審査中に散々確認した通りビルドし直さなくても配信が始まれば自動的に広告が出るようになる)。数時間~1日程度様子を見ても表示されない場合は、上記の診断ログの内容(実機のコンソール出力)を教えてもらえれば、それをもとに次の切り分けができる。
